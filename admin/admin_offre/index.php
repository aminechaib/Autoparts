...
?>
            <div class="ui fifteen wide column row centered grid segment">
                ...
                <div class="ui pointing secondary big menu">
                    ...
                    <h1 class="ui  header item"><i class="car icon"></i>Moteur</h1>
...
?>
                    ...
                    <table class="ui striped  table" id="tabAll">
                        ...
                        <tbody>
                           <?php
                               $admin_offres = Admin_offre::find_all(); 
                               if($admin_offres){
                                   foreach($admin_offres as $admin_offre){
                               ?>
                               <tr>
                                   <td><?php echo h($admin_offre->id);?></td>
                                   <td><?php echo h($admin_offre->nom_offre);?></td>
                                   <td><?php echo h($admin_offre->target_time);?></td>
                                   <td><?php echo h($admin_offre->descriptio_offre);?></td>
                                   <td><?php echo h($admin_offre->id_piece);?></td>
                                   <td>
                                       <button class="ui tiny yellow  button"
                                           data-button_id="<?php echo h($admin_offre->id) ?>" data-type="modifier"><i
                                               class="edit outline icon"></i><span>Modifier</span></button>
                                       <div class="ui modal modifier m<?php echo h($admin_offre->id, '') ?>">
                                           <div class="content">
                                               <?php modifier_modal($admin_offre->id, '') ?>
                                           </div>
                                       </div>
                                   </td>
                                   
                                   <td>
                                    <button class="ui tiny red button" data-button_id="<?php echo h($admin_offre->id) ?>"
                                        data-type="supprimer">
                                        <i class="user slash icon"></i><span>Supprimer</span></button>

                                    <div class="ui modal supprimer s<?php echo h($admin_offre->id) ?>">
                                        <?php supprimer_modal($admin_offre->id, ''); ?>
                                    </div>
                                </td>
