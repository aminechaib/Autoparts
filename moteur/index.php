...
?>
            <div class="ui fifteen wide column row centered grid segment">
                ...
                <div class="ui pointing secondary big menu">
                    ...
                    <h1 class="ui  header item"><i class="users icon"></i>Moteur</h1>
                    ...
                    <table class="ui striped  table" id="tabAll">
                        ...
                        <tbody>
                           <?php
                               ...
                               if($moteur){
                                   foreach($moteur as $mote){
                               ?>
                               <tr>
                                   <td><?php echo h($mote->id);?></td>
                                   <td><?php echo h($mote->name);?></td>
                                   <td><?php echo h($mote->enrgie);?></td>
                                   <td><?php echo h($mote->puissance);?></td>
                                   <td><?php echo h($mote->creation_date);?></td>
                                   <td>
                                       <button class="ui tiny yellow  button"
                                           data-button_id="<?php echo h($mote->id) ?>" data-type="modifier"><i
                                               class="edit outline icon"></i><span>Modifier</span></button>
                                       <div class="ui modal modifier m<?php echo h($mote->id, '') ?>">
                                           <div class="content">
                                               <?php modifier_modal($mote->id, '') ?>
                                           </div>
                                       </div>
                                   </td>
                                   
                                   <td>
                                    <button class="ui tiny red button" data-button_id="<?php echo h($mote->id) ?>"
                                        data-type="supprimer">
                                        <i class="user slash icon"></i><span>Supprimer</span></button>

                                    <div class="ui modal supprimer s<?php echo h($mote->id) ?>">
                                        <?php supprimer_modal($mote->id, ''); ?>
                                    </div>
                                </td>
